      ...
      <div class="flex-1 flex overflow-hidden">
        ...
        <div class="flex-1 overflow-y-auto bg-gradient-to-br from-gray-50 via-slate-50 to-blue-50 dark:from-slate-900 dark:via-gray-900 dark:to-slate-900 transition-colors duration-300">
          <div class="p-3 space-y-2">
            ...
            <!-- Tipo de Gestión - COMPACTO -->
            @if (showManagementType()) {
              <div class="bg-green-50 rounded-lg shadow-md border border-green-300 p-2">
                <label class="block font-bold text-gray-800 mb-1 text-[11px] flex items-center gap-1">
                  <div class="w-1.5 h-1.5 bg-red-500 rounded-full"></div>
                  Tipo de Gestión *
                </label>
                <div class="relative">
                  <select
                    [(ngModel)]="managementForm.tipoGestion"
                    (ngModelChange)="onManagementTypeChange()"
                    [class]="'w-full p-2 pr-8 border rounded-lg font-semibold text-gray-700 appearance-none cursor-pointer transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-green-200 hover:border-green-500 text-xs ' +
                      (errors().tipoGestion ? 'border-red-500 bg-red-50' : 'border-green-300 bg-white') + ' ' +
      ...
      next: (response) => {
        ...
        if (!this.callActive()) {
          ...
          this.managementService.endCall(managementId, endCallRequest).subscribe({
            next: (res) => console.log('✅ Fin de llamada registrado:', res),
            error: (err) => console.error('❌ Error al registrar fin de llamada:', err)
          });
        }
      },
      error: (error) => {
